...
void shared_cond_ctor(shared_cond_t *obj, const char *name)
{
    ...
    if (shared_mem_isowner(obj->shm))
    {
        ...
        {
            fprintf(stderr, "ERROR(%s):Could not initialize cond. attr.:%s\n", name, strerror(ret));
            exit(1);
        }
        if ((ret = pthread_condattr_setpshared(&cond_attr, PTHREAD_PROCESS_SHARED)))
        {
            fprintf(stderr, "ERROR(%s):Could not set cond. attr.:%s\n", name, strerror(ret));
            exit(1);
        }
        ...
        {
            fprintf(stderr, "ERROR(%s):Could not destroy cond. attr.:%s\n", name, strerror(ret));
            exit(1);
        }
    }
}
...
void shared_cond_dtor(shared_cond_t *obj)
{
    ...
    if (shared_mem_isowner(obj->shm))
    {
        ...
        {
            fprintf(stderr, "Error: Could not destroy shared condition variable: %s\n", strerror(ret));
            exit(1);
        }
    }
...
void shared_cond_wait(shared_cond_t *obj, struct shared_mutex_t *mutex)
{
    ...
    {
        fprintf(stderr, "Error: Could not wait on condition variable: %s\n", strerror(ret));
        exit(1);
    }
}
...
void shared_cond_timedwait(shared_cond_t *obj, struct shared_mutex_t *mutex, long int time_nanosec)
{
    ...
    {
        fprintf(stderr, "ERROR:Failed at reading real time: %s", strerror(ret));
    }
    ...
    if ((ret = pthread_cond_timedwait(obj->ptr, shared_mutex_getptr(mutex), &ts)))
    {
        ...
        if (ret == EOWNERDEAD)
        {
            fprintf(stderr, "WARN: cv's owner is dead...\n");
...
void shared_cond_broadcast(shared_cond_t *obj)
{
    ...
    {
        fprintf(stderr, "Could not broadcast cv:%s\n", strerror(ret));
    }
}